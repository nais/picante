...
func main() {
	...
	if err != nil {
		mainLogger.WithError(err).Fatal("failed to setup verify attestation verifier")
	}

	mainLogger.Info("setting up dtrack client")
	c := client.New(cfg.Storage.Api, cfg.Storage.Username, cfg.Storage.Password, client.WithApiKeySource(cfg.Storage.Team))
	if err != nil {
		mainLogger.WithError(err).Fatal("failed to get teams")
	}

	eh := monitor.NewEventHandler(ctx, c, verifier, cfg.Cluster)
	inf := informers.New(k8sClient, mainLogger, cfg.Features.LabelSelectors, eh)

	if err = inf.RunInformers(ctx); err != nil {
